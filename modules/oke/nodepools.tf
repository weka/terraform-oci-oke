...
resource "oci_containerengine_node_pool" "nodepools" {
  ...
  node_metadata = {
    ...
    user_data                = var.cloudinit_nodepool_common == "" && lookup(each.value, "cloudinit_nodepool", "") == "" ? data.cloudinit_config.worker.rendered : lookup(each.value, "cloudinit_nodepool", "") != "" ? lookup(each.value, "cloudinit_nodepool", "") : filebase64(var.cloudinit_nodepool_common)
  }
