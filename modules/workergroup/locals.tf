...
locals {
  ...
  worker_compartments = toset(distinct(compact([
    for k, v in var.worker_groups : lookup(v, "compartment_id", local.compartment_id)
    if lookup(v, "enabled", var.enabled) == true
  ])))
  ...
  worker_groups_active = merge(
    tomap(oci_core_cluster_network.cluster_networks),
    tomap(oci_core_instance_pool.instance_pools),
    tomap(oci_containerengine_node_pool.node_pools),
  )
}
