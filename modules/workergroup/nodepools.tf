...
resource "oci_containerengine_node_pool" "node_pools" {
  ...
  node_config_details {
    ...
    defined_tags = merge(
      local.defined_tags,
      lookup(each.value, "defined_tags", {}), # contains(keys(each.value), "defined_tags") ? each.value.defined_tags : {},
  ...
  lifecycle { # prevent resources changes for changed fields
    ...
    precondition {
      condition     = !(each.value.allow_autoscaler == true && each.value.autoscale == true)
      error_message = "Only one of `allow_autoscaler` and `autoscale` may be enabled for group: `${each.key}`."
    }
  }
  ...
  dynamic "initial_node_labels" {
    for_each = each.value.allow_autoscaler ? [1] : []
    content {
      key   = "app"
